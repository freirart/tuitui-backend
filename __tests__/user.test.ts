...
describe('User Sign Up flow', () => {
  ...
  it("should return an error if signing in using a username that already belongs to an user.",
  async () => {
...
describe("User Sign In flow", () => {
  ...
  it("should return an error when trying to login as an user that doesn't exist.",
  async () => {
    const response = await request(app)
      .post('/users/signin')
      .send({ username: Date.now(), password: Date.now() });
  ...
  it("should return an error when trying to login using an invalid password", 
  async () => {
    const user = await factory.create('User');
    const response = await request(app)
      .post('/users/signin')
  ...
  it("should authenticate and return a token when given valid credentials", 
  async () => {
